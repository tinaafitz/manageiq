#
# Description: This method validates the service provision request using the values
# [max_vms, max_cpus, max_memory] from values in the model
#

def log(level, msg, set_message=false)
  $evm.log(level, "#{msg}")
  $evm.root['miq_request'].set_message(msg) if $evm.root['miq_request'] && set_message
end
  ...
  parent_service_template.service_resources.each do |child_service_resource|
    ...
    if parent_service_template.service_type == 'composite'
      ...
      child_service_resource.resource.service_resources.each do |grandchild_service_template_service_resource|
        #log(:info, "Inspecting grandchild_service_template_service_resource.resource: #{grandchild_service_template_service_resource.resource.inspect}\n")
...
def max_cpu_check(service_template, options_hash, reason_hash)
  ...
  unless model_max_cpus.zero?
    ...
    if total_cpus_requested && (total_cpus_requested > model_max_cpus)
      log(:warn, "Auto-Approval Threshold: Amount of vCPUs requested: #{total_cpus_requested} exceeds: #{model_max_cpus}")
...
def max_flavor_check(service_template, options_hash, reason_hash)
  ...
  options_hash.each do |sequence_id, options|
    ...
    # model_max_flavor = nil || $evm.object['max_flavor']
    # # Validate model_max_flavor if not blank
    # unless model_max_flavor.blank?
    #   log(:info,"Auto-Approval Threshold(Model): model_max_flavor=#{model_max_flavor} detected")
    #   weight = flavor_weight[options[:flavor]] rescue 0
    #   if weight >= flavor_weight[model_max_flavor]
    #     reject = true
    #     log(:warn, "Auto-Approval Threshold: Flavor requested: #{options[:flavor]} exceeds: #{model_max_flavor}")
    #   end
    # end
