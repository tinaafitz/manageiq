...
def log(level, message, set_message=false)
  $evm.log(level, "#{message}")
  $evm.root['miq_request'].set_message(message) if $evm.root['miq_request'] && set_message
end
...
def query_prov_options(parent_service_template, prov_option)
  ...
  parent_service_template.service_resources.each do |child_service_resource|
    ...
    if parent_service_template.service_type == 'composite'
      ...
      log(:info, "Detected Service Catalog Bundle")
      child_service_resource.resource.service_resources.each do |grandchild_service_template_service_resource|
        ...
        log(:info, "Retrieving #{prov_option}=>#{grandchild_service_template_service_resource.resource.get_option(prov_option)}")
      ...
      log(:info, "Detected Service Catalog Item")
      ...
      log(:info, "Retrieving #{prov_option}=>#{child_service_resource.resource.get_option(prov_option)}")
...
def get_total_memory_requested(service_template, options_hash)
  ...
  log(:info, "total_memory_requested: #{total_memory_requested}", true)
...
def check_quotas(miq_request, entity, quota_hash)
  ...
  log(:info, "#{entity_type}: #{entity_name} current vRAM allocated (bytes): #{entity_consumption[:memory]} current vRAM allocated (megabytes): #{entity_consumption[:memory] / 1024**2}")
  log(:info, "#{entity_type}: #{entity_name} current VMs: #{entity_consumption[:vms]}")
  log(:info, "#{entity_type}: #{entity_name} current Storage Allocated (bytes): #{entity_consumption[:allocated_storage]} vRAM allocated (gigabytes): #{number_to_human_size(entity_consumption[:allocated_storage], :precision => 2)}")
  log(:info, "#{entity_type}: #{entity_name} current Storage Provisioned (bytes): #{entity_consumption[:provisioned_storage]} Storage Provisioned (gigabytes): #{number_to_human_size(entity_consumption[:provisioned_storage], :precision => 2)}")
  ...
  # Memory Quota Check
  ##########
  log(:info, "#{entity_type}: #{entity_name} current vRAM allocated (megabytes): #{entity_consumption[:memory] / 1024**2}")
  ...
  unless quota_max_memory.zero?
    if entity_consumption && (entity_consumption[:memory] / 1024**2 + quota_hash[:total_memory_requested] > quota_max_memory)
      log(:info, "#{entity_type}: #{entity_name} current vRAM allocated (megabytes): #{entity_consumption[:memory] / 1024**2} + requested: #{quota_hash[:total_memory_requested]} exceeds quota: #{quota_max_memory}")
      quota_hash[:quota_exceeded] = true
...
# require action_view to be able to convert numbers to human sizes and to trucnate long strings
require 'action_view'
