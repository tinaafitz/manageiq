class ServiceTemplateProvisionTask < MiqRequestTask
  ...
  def after_ae_delivery(ae_result)
    ...
    if ae_result == 'ok'
      msg = self.get_option(:user_message) || "#{self.request_class::TASK_DESCRIPTION} completed"
      update_and_notify_parent(:state => "finished", :status => "Ok", :message => msg)
    else
      mark_pending_items_as_finished
      msg = self.get_option(:user_message) || "#{self.request_class::TASK_DESCRIPTION} failed"
      update_and_notify_parent(:state => "finished", :status => "Error", :message => msg)
    end
  end
